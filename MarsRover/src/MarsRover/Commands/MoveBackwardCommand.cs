...
public class MoveBackwardCommand : IRobertCommand {
    
    public SimulatedExecutionResult SimulateExecution(MapCoordinates map, Router router) {
        try {
            var newLocation = map.CurrentLocation.Move(router.CurrentDirection, Movement.Backward); ;
    ...
    public CommandExecutionResult Execute(MapCoordinates map, Router router) {
        MoveWheels(Movement.Backward);
        var newLocation = ReadNewLocationUsingSensors(map.CurrentLocation, router.CurrentDirection, Movement.Backward);
